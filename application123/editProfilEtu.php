...
if($validator->isValide()){
    ...
    $users_id =  Session::getInstance()->read('etud')->idEtu;
    if(isset($_POST['pseudo']) AND isset($_POST['nom']) AND isset($_POST['prenom'])){
     $req = $db->query('UPDATE  etudiants SET pseudoEtu = ?,nomEtu= ?,prenomEtu=?,photoEtu=? WHERE idEtu = ?',[$_POST['pseudo'] , $_POST['nom'],$_POST['prenom'],'photos/'.$_FILES['photo']['name'],$users_id]);
     // faire une requette a la base de donnees
     $user = $db->query('SELECT * FROM etudiants WHERE idEtu= ?',[$users_id])->fetch();
        
     Session::getInstance()->write('etud',$user);
...
<div class="col-lg-6" >
			...
			<?php if(array_key_exists('nom',$errors)):?>
                   ...
                   <?php endif;?>
            <label for="nom">    <?=  Session::getInstance()->read('etud')->nomEtu ?></label>
		</div>
        ...
        <label for="prenom">prenom</label>
            ...
            <?php if(array_key_exists('prenom',$errors)):?>
                   ...
                   <?php endif;?>
            <label for="prenom"><?=  Session::getInstance()->read('etud')->prenomEtu ?></label>
		</div>
        <div class="form-group">
        <label for="pseudo">pseudo</label>
            ...
            <?php if(array_key_exists('pseudo',$errors)):?>
                   ...
                   <?php endif;?>
            <label for="pseudo"><?=  Session::getInstance()->read('etud')->pseudoEtu ?></label>
		</div>
