...
require_once 'inc/functions.php';
  ...
  $validator->isUniq('pseudo',$db,'idEns','pseudoEns','enseignants', "ce pseudo est deja pris");
  $validator->isEmail('email',"Votre email est invalide","Champ obligatoire");
  $validator->isUniq('email',$db,'idEns','emailEns','enseignants',"cet email est deja utilisé pour un autre compte" );
...
if($validator->isValide()){
  ...
  if(isset($_POST['pseudo']) AND isset($_POST['passwords']) AND isset($_POST['email']) AND isset($_POST['dateN'])  AND isset($_POST['nom']) AND isset($_POST['prenom']) AND isset($_POST['adresse']) AND isset($_POST['domaines'])){ 
    $password = password_hash($_POST['passwords'], PASSWORD_BCRYPT);
    $token =  Str::random(60);
    $db->query('INSERT INTO enseignants SET pseudoEns = ?,passwordEns = ?,emailEns = ?,date_NEns = ?,nomEns= ?,prenomEns=?, adresseEns = ?,confirmation_token =?,photoEns = ?,domainesENS = ?,descriptionEns= ?',[$_POST['pseudo'], $password, $_POST['email'],$_POST['dateN'],$_POST['nom'],$_POST['prenom'],$_POST['adresse'],$token,'photos/'.$_FILES['photo']['name'],$_POST['domaines'],$_POST['description']]);
    
$Ens_id = $db->lastInsertId();
    $db->query('INSERT INTO specialites SET nomSpe = ?, idEns = ?',[$_POST['domaines'],$Ens_id]);
$headers = "From: Sendmail Tests" . PHP_EOL;
$headers .= 'Content-type: text/html; charset=utf-8' . PHP_EOL;
mail($_POST['email'],'confirmation de votre compte',"Afin de valider votre compte merci de cliquer sur ce lien\n\nhttp://localhost/application/confirm.php?idEn=$Ens_id&tokenEn=$token",$headers);
 ...
 <div class="welll">
    ...
    <div class="col-lg-8">
                ...
                <div class="col-lg-4">      
                   <div class="form-group">
                             ...
                             <div class="requis"><?php echo $errors['dateN']?></div>
                               <?php endif;?>
                              </div> 
                </div>              
    </div>
   ...
   <div class="row">
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                ...
                <input type="passwords" name="passwords_confirm" class="form-control" >
    ...
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
               <div class="form-group">
               <label for="inputState">Domaines</label>
               <select id="inputState" name="domaines" class="do form-control">
               <?php if(array_key_exists('domaines',$errors)):?>
                 <div style="color:red;"><?php echo $errors['domaines']?></div>
                 <?php endif;?>
                <?php foreach($compitences as $comp):?>
                <option><?php echo $comp; ?></option>
                <?php endforeach;?>
               </select>
              </div>

               <div class="form-group">
               <label for="comment">Déscritpion:</label><br>
               <textarea class="f" rows="5" id="comment" name="description">
